...
\begin{itemize}
    ...
    \begin{multline*}
            \forall \varepsilon > 0\ \exists \delta > 0\ \forall (x,y) \in \mathbb{R}^2 :\\ 0 < \sqrt{(x-a)^2+(y-b)^2} < \delta \Rightarrow |f(x,y) - A| < \varepsilon
    \end{multline*}
    \item Funkcija je \textbf{zvezna} v točki $(a,b)$, če
    \begin{multline*}
            \forall \varepsilon > 0\ \exists \delta > 0\ \forall (x,y) \in \mathbb{R}^2 :\\ 0 < \sqrt{(x-a)^2+(y-b)^2} < \delta \Rightarrow |f(x,y) - f(a,b)| < \varepsilon
    \end{multline*}
    \item Če je $f$ zvezna v točiki $(a,b)$, je
    \[\lim_{(x,y) \to (a,b)} f(x,y) = f(a,b) \]
\end{itemize}
\subsubsection{Smerni odvod}
\[\frac{\partial f}{\partial \vect{s}}(x,y) = \lim_{t \to 0} \frac{f((x,y) + t\vect{s}\,)}{t}\]
